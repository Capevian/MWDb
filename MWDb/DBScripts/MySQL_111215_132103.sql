...
alter table `comentario`  add column  `oid`  integer;
alter table `miniatura`   add index fk_comentario_miniatura (`oid`), add constraint fk_comentario_miniatura foreign key (`oid`) references `miniatura` (`oid`);
create index `idx_comentario_miniatura` on `miniatura`(`oid`);


-- Comentário_User [rel7]
alter table `comentario`  add column  `oid`  integer;
alter table `user`   add index fk_comentario_user (`oid`), add constraint fk_comentario_user foreign key (`oid`) references `user` (`oid`);
create index `idx_comentario_user` on `user`(`oid`);
